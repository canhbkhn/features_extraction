...
def feature_evaluate1(pdf_train, pdf_feat, ls_feat=None):
    out_res = {
        "name": [],
        "auc": [],
        "corr": [],
...
def feature_evaluate(pdf_train, pdf_feat, ls_feat=None):
    ...
    pdf_eval = pdf_train.merge(pdf_feat, on="id")
    if ls_feat is None:
        ls_feat = [cname for cname in pdf_feat.columns if cname != "id"]
    ...
    for feat in ls_feat:
        out_res["name"].append(feat)
        out_res["auc"].append(my_auc(pdf_eval[feat], pdf_eval["label"]))
